...
- Sampled verification materializes each final candidate repository and runs two independent falsification passes that can inspect files and execute focused read-only checks, with the second pass explicitly challenging the first pass's conclusions. It then gives the combined audits to the pairwise judge as untrusted leads. Judgments prioritize semantic contract correctness over candidate-authored validation, retain recorded tool calls and results, and omit assistant reasoning and final claims. Claimed bugs must survive code-path falsification and produce a contract-valid counterexample. Rankings use pairwise-majority wins, weakest-head-to-head strength, then expected probability.
