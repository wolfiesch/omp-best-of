...
- A discoverable `best_of` extension tool exposes the existing runner to model calls, with arbitrary OMP candidate and verifier selectors, session model and thinking inheritance, opt-in patch application, cancellation, progress updates, and compact artifact-backed results.
- The installed package now bundles a `bestof` skill that parses implementer and reviewer roles from live OMP configuration and calls `best_of`; explicit reviewers default to the sampled backend so any available OMP model can judge.
